...
\begin{frame}[c]{Surrogate modelling}
    ...
    \includegraphics<+>[width=\textwidth]{figures/surrogate-2.pdf}%
...
\begin{frame}[t]{How do we assess performance?}
	...
	\begin{cbox}{KCLpeagreen}{1.\ Discrimination}

		{\large
            Within 0.1 of the achievable performance without sample size constraints,
            $P^*$}

		\bgap

		The choice of metric and metric can be set by users\\
        \R{minimum_threshold = 0.1}
        \R{metric = "auc"}
        

	\end{cbox}

	\begin{cbox}{KCLseablue}{2.\ Calibration}
        {\large
...
\begin{frame}[t]{Two approaches to estimating minimum samples}
	...
	\Wider[5em]{
		\begin{tabular}{lp{0.4\textwidth}p{0.4\textwidth}}
			...
			Find sample size such that with 80\% probability test performance is above $P^{*}$                                                                                                                            \\[14mm]
		...
		\begin{tabular}{p{0.5\textwidth}p{0.5\textwidth}}
			...
			\begin{minipage}[t]{0.4\textwidth}
				\begin{itemize}
					...
					\item Flexible, handles machine learning or multilevel
					      models, etc.
					      % heteroscedasticity and multicollinearity in the data
					\item Does not aim to prevent overfitting per se
					\item Targets test performance itself *
					\item Adjusts recommendations to the test performance
					      variance **
				\end{itemize}
				However:\
				\begin{itemize}
					\item Takes time / computational resources
					\item Requires user input including the prediction model
					      and/or data generator for complex designs
					\item Depends on simulations, so variability of results is
					      expected
				\end{itemize}
